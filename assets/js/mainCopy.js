...
async function getMargins() {
    let orderMarginValues = getOrderValues(document.getElementById('optionBuyButton'));
    orderMarginValues.trantype = "B";
...
function getOrderValues(button) {
...
async function placeOrder(button) {
    let orderValues = getOrderValues(button);
...
async function modifyOrder(button) {
    let orderValues = getOrderValues(button);
    orderValues.norenordno = document.getElementById("optionName").getAttribute("data-norenordno");
...
function handleOrderRow(orderRow, trantype) {
    return {
        uid: localStorage.getItem("uid"),
        actid: localStorage.getItem("actid"),
        exch: orderRow.childNodes[3].childNodes[0].textContent.slice(-3),
...
async function cloneOrder(cloneButton) {
    ...
    let trantype = orderRow.childNodes[4].textContent > 0 ? "B" : "S";
    let orderValues = handleOrderRow(orderRow, trantype);
    await shoonyaApi(orderValues, "PlaceOrder");
    positions();
}
...
async function exitSelectedButton() {
    ...
    setTimeout(() => {
        rows.forEach((orderRow, i) => {
            setTimeout(
                function () {
                    let trantype = orderRow.childNodes[4].textContent > 0 ? "S" : "B";
                    let orderValues = handleOrderRow(orderRow, trantype)
